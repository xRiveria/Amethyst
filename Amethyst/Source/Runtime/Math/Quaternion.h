...
namespace Amethyst::Math
{
    ...
    public:
        ...
        static inline Quaternion FromAngleAxis(float angle, const Vector3& axis)
        {
            ...
            return Quaternion(axis.m_X * sin, axis.m_Y * sin, axis.m_Z * sin, cos);
        }
        ...
        static inline auto FromEulerAngles(const Vector3& rotation) { return FromYawPitchRoll(rotation.m_Y * Utilities::DegreeToRadian, rotation.m_X * Utilities::DegreeToRadian, rotation.m_Z * Utilities::DegreeToRadian); }
        static inline auto FromEulerAngles(float rotationX, float rotationY, float rotationZ) { return FromYawPitchRoll(rotationY * Utilities::DegreeToRadian, rotationX * Utilities::DegreeToRadian, rotationZ * Utilities::DegreeToRadian); }
        ...
        Vector3 ToEulerAngles() const
        {
            ...
            if (check < -0.995f)
            {
                ...
                (
                    -90.0f,
                    0.0f,
                    -atan2f(2.0f * (x * z - w * y), 1.0f - 2.0f * (y * y + z * z)) * Utilities::RadianToDegree
                );
            }
            ...
            return Vector3
            (
                asinf(check) * Utilities::RadianToDegree,
                atan2f(2.0f * (x * z + w * y), 1.0f - 2.0f * (x * x + y * y)) * Utilities::RadianToDegree,
                atan2f(2.0f * (x * y + w * z), 1.0f - 2.0f * (x * x + z * z)) * Utilities::RadianToDegree
            );
        }

        // Returns yaw in degrees
        auto Yaw() const { return ToEulerAngles().m_Y; }
        // Returns pitch in degrees
        auto Pitch() const { return ToEulerAngles().m_X; }
        // Returns roll in degrees
        auto Roll() const { return ToEulerAngles().m_Z; }
        ...
        static inline Quaternion FromToRotation(const Vector3& start, const Vector3& end)
        {
            ...
            else
            {
                ...
                if (axis.Length() < Utilities::Episilon)
                {
                    axis = Vector3::Up.Cross(normStart);
                }

                return FromAngleAxis(180.0f * Utilities::DegreeToRadian, axis);
            }
        }
        ...
        static inline Quaternion FromLookRotation(const Vector3& direction, const Vector3& upDirection = Vector3::Up)
        {
            ...
            if (v.LengthSquared() >= Utilities::Episilon)
            {
                v.Normalize();
        ...
        void Normalize()
        {
            ...
            if (!Utilities::Equals(length_squared, 1.0f) && length_squared > 0.0f)
            {
                const auto length_inverted = 1.0f / Utilities::SquareRoot(length_squared);
                x *= length_inverted;
                y *= length_inverted;
                z *= length_inverted;
                w *= length_inverted;
        ...
        Quaternion Inverse() const
        {
            ...
            if (length_squared == 1.0f)
                return Conjugate();
            else if (length_squared >= Utilities::Episilon)
        ...
        {
            return Utilities::Equals(x, rhs.x) && Utilities::Equals(y, rhs.y) && Utilities::Equals(z, rhs.z) && Utilities::Equals(w, rhs.w);
        }
