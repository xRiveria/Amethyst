...
public:
	...
	{
		if (!Amethyst::FileSystem::IsDirectory(directory))
		{
			return false;
		}
		...
		else //If there is a slash, get the individual directories between slashes.
		{
			...
			while (true)
			{
				...
				m_PathHierarchy.emplace_back(Amethyst::FileSystem::RetrieveFileNameFromFilePath(m_CurrentPath.substr(0, slashPositionIndex))); 
				...
				{
					m_PathHierarchy.emplace_back(Amethyst::FileSystem::RetrieveFileNameFromFilePath(m_CurrentPath));
					break;
				}
			}
		}
...
public:
	FileDialogItem(const std::string& filePath, const Icon& itemIcon)
	{
		...
		this->m_ID = Amethyst::AmethystObject::GenerateObjectID();
		this->m_IsDirectory = Amethyst::FileSystem::IsDirectory(filePath);
		this->m_HierarchyLabel = Amethyst::FileSystem::RetrieveFileNameFromFilePath(filePath);
	}
	...
	Amethyst::RHI_Texture* RetrieveItemTexture() const { return IconLibrary::RetrieveIconLibraryInstance().RetrieveTextureByIcon(m_Icon); }
