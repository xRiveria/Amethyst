...
FileDialog::FileDialog(bool isStandaloneWindow, FileDialog_Type type, FileDialog_Operation operation, FileDialog_Filter filter)
{
	...
	m_Navigation.NavigateTo(Amethyst::FileSystem::RetrieveWorkingDirectory());
}
...
{
	std::vector<std::string> directories = Amethyst::FileSystem::RetrieveDirectoriesInDirectory(directoryPath);

	for (int i = 0; i < directories.size(); i++)
	{
		if (ImGui::TreeNodeEx(Amethyst::FileSystem::RetrieveFileNameFromFilePath(directories[i]).c_str()))
		{
			for (int z = 0; z < Amethyst::FileSystem::RetrieveFilesInDirectory(directories[i]).size(); z++)
			{
				ImGui::TreeNodeEx(Amethyst::FileSystem::RetrieveFileNameFromFilePath(Amethyst::FileSystem::RetrieveFilesInDirectory(directories[i])[z]).c_str(), ImGuiTreeNodeFlags_Leaf);
				ImGui::TreePop();
			}
			...
			ImGui::BeginGroup(); //Group that makes up each of our item.
			{
				...
				//Icon
				{
					...
					if (ImGui::Button("##Dummy", m_DialogItemSize))
					{
						...
						else //Double Click
						{
							...
							if (m_Type == FileDialog_Type_Browser)
							{
								if (!item.IsDirectory())
								{
									Amethyst::FileSystem::OpenDirectoryWindow(item.RetrieveFilePath());
								}
							}
					...
					//Image
					{
						...
						ImVec2 imageSize = item.RetrieveItemTexture() ? ImVec2(static_cast<float>(item.RetrieveItemTexture()->RetrieveWidth()), static_cast<float>(item.RetrieveItemTexture()->RetrieveHeight())) : imageSizeMax;
						...
						ImGui::Image(item.RetrieveItemTexture(), imageSize);
					}
...
void FileDialog::ItemContextMenu(FileDialogItem* dialogItem)
{
	...
	if (ImGui::MenuItem("Delete"))
	{
		...
		{
			Amethyst::FileSystem::Delete(dialogItem->RetrieveFilePath());
			m_IsDirty = true;
		}
		else
		{
	...
	{
		Amethyst::FileSystem::OpenDirectoryWindow(dialogItem->RetrieveFilePath());
	}
...
{
	if (!Amethyst::FileSystem::IsDirectory(directoryPath))
	{
	...
	std::vector<std::string> childDirectories = Amethyst::FileSystem::RetrieveDirectoriesInDirectory(directoryPath);
	for (const std::string& childDirectory : childDirectories)
	{
		m_DialogItems.emplace_back(childDirectory, IconLibrary::RetrieveIconLibraryInstance().LoadIcon_(childDirectory, IconType::Icon_AssetBrowser_Folder, static_cast<int>(m_DialogItemSize.x)));
	}

	//Retrieve Items
	std::vector<std::string> childItems = Amethyst::FileSystem::RetrieveFilesInDirectory(directoryPath);
	for (const std::string& childItem : childItems)
	{
		if (Amethyst::FileSystem::IsEngineFile(childItem) || Amethyst::FileSystem::IsSupportedImageFile(childItem))
		{
			m_DialogItems.emplace_back(childItem, IconLibrary::RetrieveIconLibraryInstance().LoadIcon_(childItem, IconType::Icon_Custom, static_cast<int>(m_DialogItemSize.x)));
		}
	}
...
void FileDialog::AssetBrowserContextMenu()
{
	...
	{
		Amethyst::FileSystem::CreateDirectory_(m_Navigation.m_CurrentPath + "/New Folder");
		m_IsDirty = true;
	}
	...
	{
		Amethyst::FileSystem::OpenDirectoryWindow(m_Navigation.m_CurrentPath);
	}
